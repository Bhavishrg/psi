...
common::utils::Circuit<Ring> generateGraphitiInitCircuit(int nP, int pid, size_t num_verts, size_t num_edges, size_t num_inserts) {
    ...
    std::cout << "  vec_size=" << vec_size << " (num_verts + num_edges + num_inserts)" << std::endl;
...
void benchmark(const bpo::variables_map& opts) {
    ...
    auto circ = generateGraphitiInitCircuit(nP, pid, num_verts, num_edges, num_inserts).orderGatesByLevel();
    network->sync();
...
bpo::options_description programOptions() {
    ...
    desc.add_options()
        ...
        ("num-inserts", bpo::value<size_t>(), "Number of vertex insertions (default: num-verts * 0.05).")
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (opts.count("num-inserts") == 0) {
            ...
            size_t default_inserts = static_cast<size_t>(num_verts * 0.05);
